...
async function getSupabaseURL(): Promise<string> {
	console.log('Step 2: Supabase API Endpoint');
	return await question('Enter your SUPABASE_API_URL: ');
}

async function getSupabaseAnonKey(): Promise<string> {
	console.log('Step 2: Supabase Anon Key');
	return await question('Enter your SUPABASE_ANON_KEY: ');
}
...
async function main() {
	...
	const PUBLIC_SUPABASE_URL = await getSupabaseURL();
	const PUBLIC_SUPABASE_ANON_KEY = await getSupabaseAnonKey();
